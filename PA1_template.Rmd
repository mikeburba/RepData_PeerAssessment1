...
```
The mean number of steps is `r total_steps_mean` and the median is `r total_steps_median`. Here's where they fall on the histogram of Total Steps Per Day:
```{r}
tsp <- tsp + 
...
Our subject tends to exert their max activity at interval `r max_interval`.
...
This data is missing steps values for `r num_missing_steps` intervals.
...
The biggest difference, as you can see from the chart, is that the large number of days with < 700 steps is eliminated. those days have been shifted to the mean, which is just over 10,000 steps. Further, the mean (`r imputed_total_steps_mean`) and the median (`r imputed_total_steps_median`) are almost exactly equal. 
